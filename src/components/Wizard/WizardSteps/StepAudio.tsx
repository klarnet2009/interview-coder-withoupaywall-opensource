...
import { Mic, Monitor, Check, Volume2, AlertCircle } from 'lucide-react';
...
/*
 * There is deliberately no per-application entry. Chromium ignores the window id for
 * the audio track on Windows, so that option named one app and captured everything.
 * See electron/constants/audioSource.ts.
 */
...
export const StepAudio: React.FC<StepAudioProps> = ({
  ...
  useEffect(() => {
    // Both surviving sources are always selectable, so this step never blocks.
    setCanProceed(true);
  ...
  }, [selectedSource, autoStart, testCompleted, onUpdate, setCanProceed]);

  // Changing source invalidates previous test result.
  useEffect(() => {
